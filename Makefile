...
PG_CPPFLAGS += $(shell pkg-config --cflags-only-I openssl 2>/dev/null)
SHLIB_LINK += $(shell pkg-config --libs openssl 2>/dev/null || echo "-lssl -lcrypto")
PG_CPPFLAGS += -DWITH_SSL=1 -DENABLE_THREAD_SAFETY=1 -DHAVE_CONFIG_H=1
...
$(LIBRABBITMQ_STAMP):
	...
	@echo '#if defined(__linux__) || defined(__unix__) || defined(__APPLE__)' >> $(LIBRABBITMQ_DIR)/config.h
	...
	@echo '#ifdef WITH_SSL' >> $(LIBRABBITMQ_DIR)/config.h
	@echo '#define AMQP_WITH_SSL 1' >> $(LIBRABBITMQ_DIR)/config.h
	@echo '#endif' >> $(LIBRABBITMQ_DIR)/config.h
	@echo '#ifdef ENABLE_THREAD_SAFETY' >> $(LIBRABBITMQ_DIR)/config.h
